...
for j in range(min_number_of_objects):
    ...
    for k in range(len(final_wave)):
        ...
        stack_wave.append(np.nanmean(data_to_stack['Flux_nonoise'][:, k]))
        stack_wave.append(np.nanmean(data_to_stack['Flux_nonoise_infres'][:, k]))
        stack_wave.append(np.nanmean(data_to_stack['corresponding_data_flux'][:, k]))
