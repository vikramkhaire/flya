...
def get_tau_avg_data(beta, forward_file, tau_limits,  sightlines = None, find_error = True, seed = 42):
    np.random.seed(seed)
    ...
    if sightlines is None:
        ...
        if sightlines <= len(data):
            ...
            data = data[0: sightlines]

            new_flux = data['Flux'][data['Mask']==0]
    ...
    if find_error:
        ...
        for _ in range(200):
            ...
            for i in bootsample:
                new_data = data[i]
                new_flux.append(new_data['Flux'][new_data['Mask']==0])
...
def diffuse_lya_fraction_forward_danforth(taufile, forward_file, sightlines =50,  Gamma_HI = None, simname = 'tng',
                                          tau_def = 'Dave', z_data = 0.03, find_errors = True):
    ...
    tau_avg, tau_avg_perfect, bootmean, boot_std  = get_tau_avg_data(
        beta=beta, forward_file=forward_file, sightlines =sightlines, tau_limits=tau_limits, find_error= find_errors)
