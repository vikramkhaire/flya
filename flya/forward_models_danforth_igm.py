...
def do_danforth_fwd_z003(path, Gamma_12, SN =60, nskew = 200):
    ...
    forward_model_filename = path + '/flya/danforth_sn/forward_model_igm_danforth.fits'
    ...
    data, resolution_array, z_qso_array = data_readin.data_readin(
        use_metalmasking=False, min_z=0.005, max_z=0.06, dataset='COS_data', SN = SN, path=datadir, fill_with_noise=True, use_emissionmasking=True)

    model_all_effects, model_in_lengthened_only, res_array, ind = forward_model.forward_model(model,data=data, data_resolution=resolution_array)
